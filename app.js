...
function showUploadOverlay(title, filesArray) {
    const titleEl = document.getElementById('globalUploadTitle');
    const textEl = document.getElementById('globalUploadText');
    if (titleEl) titleEl.innerText = title || 'Processing...';
    if (textEl) textEl.innerText = filesArray ? `0 of ${filesArray.length} Files Uploaded` : 'Starting...';
    if(document.getElementById('globalUploadBar') && document.getElementById('globalUploadBar').parentElement) document.getElementById('globalUploadBar').parentElement.style.display = 'none';
...
function showCustomDialog(title, message, isConfirm, onConfirmCallback) {
    ...
    if (btnContainer) {
        ...
        if(isConfirm) {
            ...
            const confBtn = document.getElementById('dialogConfirmBtn');
            if (confBtn) confBtn.onclick = () => { closeDialog(); if(onConfirmCallback) onConfirmCallback(); };
        } else {
    ...
    const diag = document.getElementById('customDialog');
    if (diag) diag.classList.remove('hidden');
}
function closeDialog() { const diag = document.getElementById('customDialog'); if (diag) diag.classList.add('hidden'); }
...
window.handleNextOrLogin = function() {
    ...
    const loginBtn = document.getElementById("loginBtn");
    if (!idVal) { if (statusEl) { statusEl.style.display = "block"; statusEl.innerText = "Enter ID first"; } return; }
    if (!pwd) { if (statusEl) { statusEl.style.display = "block"; statusEl.innerText = "Enter Password"; } return; }
    ...
    .then(res => {
        ...
        if (res && res.user) {
            ...
            if (statusEl) statusEl.innerText = res.message || "Invalid Login";
        }
    }).catch(err => {
        if (loginBtn) loginBtn.disabled = false;
        if (statusEl) statusEl.innerText = "Server Error: Make sure API_URL is correct.";
    });
};
...
window.logoutUser = function() {
    ...
    const logV = document.getElementById("loginView");
    ...
    if (appV) appV.classList.add("hidden");
    if (logV) logV.classList.remove("hidden");
    if (emF) emF.value = "";
    if (pwF) pwF.value = "";
...
window.searchInDropdown = function(input, containerId) {
    ...
    dropdownSearchTimeout = setTimeout(() => {
        const dropBox = document.getElementById(containerId);
        if (dropBox) {
            const items = dropBox.querySelectorAll('.dropdown-item');
            items.forEach(item => {
...
window.selectAllInDropdown = function(containerId) { const dropBox = document.getElementById(containerId); if (dropBox) { dropBox.querySelectorAll('.dropdown-item').forEach(item => { if (item.style.display !== 'none') { const cb = item.querySelector('input[type="checkbox"]'); if (cb) cb.checked = true; } }); } };
window.clearAllInDropdown = function(containerId) { const dropBox = document.getElementById(containerId); if (dropBox) { dropBox.querySelectorAll('.dropdown-item').forEach(item => { if (item.style.display !== 'none') { const cb = item.querySelector('input[type="checkbox"]'); if (cb) cb.checked = false; } }); } };
...
window.applyLookerFilters = function(containerId, type) {
    ...
    const appliedBoxes = Array.from(container.querySelectorAll('input[type="checkbox"][data-applied="true"]'));
...
window.processBulkArchive = function() {
    ...
    showCustomDialog("Confirm Archive", `Are you sure you want to archive ${selectedIds.length} selected case(s)?\n\nCase IDs: \n${displayIds}`, true, async () => {
        ...
        try { 
            ...
            if (btn) { btn.innerText = "Archive Selected"; btn.disabled = false; }
        }
    });
};
    ...
    showCustomDialog(
        ...
        async () => {
            ...
            try { 
                ...
                if (btn) { btn.innerText = "📦 Archive"; btn.disabled = false; }
            }
        }
    );
};
...
window.processUnarchive = async function(btn) {
    ...
    if(!convId) return;
    if (btn) { btn.innerText = "Unarchiving..."; btn.disabled = true; }
    ...
    catch(e) { showCustomDialog("Error", "Failed to unarchive.", false); if (btn) { btn.innerText = "📂 Un-Archive"; btn.disabled = false; } }
};
...
window.processUnsnooze = async function(btn) {
    ...
    const convId = parent ? String(parent.dataset.convId).trim() : document.getElementById('detail-conv-id')?.value;
    if(!convId) return;
    if (btn) { btn.innerText = "Un-snoozing..."; btn.disabled = true; }
    ...
    catch(e) { showCustomDialog("Error", "Failed to un-snooze.", false); if (btn) { btn.innerText = "🔔 Un-Snooze"; btn.disabled = false; } }
};
...
window.closeManageMembers = function() { const modal = document.getElementById('manageMembersModal'); if (modal) modal.classList.add('hidden'); document.getElementById('member_search_input').value = ''; document.getElementById('member_search_dropdown').classList.add('hidden'); };
...
window.saveManagedMembers = async function() {
    const btn = document.getElementById('saveMembersBtn'); if (btn) { btn.innerText = "Saving..."; btn.disabled = true; }
    const convId = document.getElementById('detail-conv-id').value;
    try {
    ...
    } catch(e) { showCustomDialog("Error", "Failed to update members", false); } finally { if (btn) { btn.innerText = "Save Changes"; btn.disabled = false; } }
};
...
window.toggleAudioRecording = async function() {
    ...
    if (mediaRecorder && mediaRecorder.state === "recording") {
        mediaRecorder.stop();
        if (micBtn) { micBtn.classList.remove('text-red-600', 'bg-red-100'); micBtn.classList.add('text-slate-500'); }
        if (recordingUI) { recordingUI.classList.add('hidden'); recordingUI.classList.remove('flex'); }
    ...
    try {
        ...
        if (micBtn) { micBtn.classList.remove('text-slate-500'); micBtn.classList.add('text-red-600', 'bg-red-100'); }
        if (recordingUI) { recordingUI.classList.remove('hidden'); recordingUI.classList.add('flex'); }
...
window.toggleInlineAudioRecording = async function(btn) {
    ...
    if (mediaRecorder && mediaRecorder.state === "recording") {
        mediaRecorder.stop();
        if (btn) { btn.classList.remove('text-red-600', 'bg-red-100'); btn.classList.add('text-slate-500'); }
        if (recordingUI) { recordingUI.classList.add('hidden'); recordingUI.classList.remove('flex'); }
        clearInterval(recordTimerInterval);
        return;
    }
    try {
        ...
        if (btn) { btn.classList.remove('text-slate-500'); btn.classList.add('text-red-600', 'bg-red-100'); }
        if (recordingUI) { recordingUI.classList.remove('hidden'); recordingUI.classList.add('flex'); }
...
window.toggleNewCaseAudioRecording = async function(btn) {
    ...
    if (mediaRecorder && mediaRecorder.state === "recording") {
        ...
        if (textSpan) textSpan.innerText = "Record Audio";
    ...
    try {
        ...
        if (textSpan) textSpan.innerText = "Stop Recording";
        const micIcon = btn.querySelector('i');
...
window.selectReplyMentionUser = function(name, email) {
    ...
    if (isAdmin || isCreator) window.finalizeReplyMention(name, email, 'Admin');
        ...
        if (!window.currentCaseHasAdminRights) {
            ...
            if (dropdown) dropdown.classList.add('hidden'); return;
        }
        if (dropdown) dropdown.innerHTML = `<div class="bg-slate-800 px-3 py-2 text-xs font-bold text-white">Select Role for ${name}</div> <div onclick="finalizeReplyMention('${String(name).replace(/'/g, "\\'")}', '${String(email).replace(/'/g, "\\'")}', 'Admin')" class="p-2 hover:bg-blue-50 cursor-pointer border-b text-sm font-bold text-blue-700">👑 Admin</div> <div onclick="finalizeReplyMention('${String(name).replace(/'/g, "\\'")}', '${String(email).replace(/'/g, "\\'")}', 'User')" class="p-2 hover:bg-slate-50 cursor-pointer text-sm font-medium text-slate-700">👤 User</div>`;
    }
};
...
window.finalizeReplyMention = function(name, email, role) {
    ...
    if (!isAdmin && !isUser && !isCreator) {
        ...
        const convId = document.getElementById('detail-conv-id')?.value;
        const cardEl = convId ? document.querySelector(`[data-conv-id="${window.normalizeCaseId(convId)}"]`) : null;
        const archivedByStr = cardEl ? (cardEl.dataset.archivedBy || '') : '';
        const snoozeTimeStr = cardEl ? (cardEl.dataset.snoozeRawStr || '') : '';

        const badgeHtml = window.getMemberBadgeHTML(email, role, archivedByStr, snoozeTimeStr);

        if (role === 'Admin' && detAdm) { detAdm.insertAdjacentHTML('afterbegin', badgeHtml); }
        else if (detUsr) { detUsr.insertAdjacentHTML('afterbegin', badgeHtml); }
...
window.setReplyGlobalType = function(type) {
    ...
    if(type === 'Message') {
        if (btnReply) btnReply.className = "px-2 sm:px-3 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-xs font-extrabold transition-all bg-white shadow-sm border border-slate-200 text-slate-800";
        if (btnAsk) btnAsk.className = "px-2 sm:px-3 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-xs font-extrabold transition-all text-slate-500 hover:text-slate-700 hover:bg-slate-50";
    } else {
        if (btnAsk) btnAsk.className = "px-2 sm:px-3 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-xs font-extrabold transition-all bg-white shadow-sm border border-slate-200 text-red-700";
        if (btnReply) btnReply.className = "px-2 sm:px-3 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-xs font-extrabold transition-all text-slate-500 hover:text-slate-700 hover:bg-slate-50";
    }
    renderReplyDynamicUI();
};
...
function renderReplyDynamicUI() {
    ...
    if (!container || !globalSelector) return;
...
window.openReplyComposerMode = function(mode) { replyComposerState.mode = mode; renderReplyDynamicUI(); };
...
window.openEditCaseModal = function() {
    const sEl = document.getElementById('detail-subject');
    const dEl = document.getElementById('detail-details');
    const lEl = document.getElementById('detail-labels');
    if (sEl) document.getElementById('edit_subject').value = sEl.innerText;
    if (dEl) document.getElementById('edit_details_rich').innerHTML = dEl.innerHTML;
    if (lEl) currentEditLabels = new Set(Array.from(lEl.children).map(span => span.innerText));
    renderEditLabels();
    const convId = document.getElementById('detail-conv-id').value;
...
function renderEditAttachments() {
    const cAt = document.getElementById('edit_current_attachments');
...
window.openCaseDetail = async function(cardEl) {
    ...
    try {
        ...
        if (status === 'Archived' && unarchiveBtn) {
        ...
        if (status !== 'Archived') {
            ...
            else if (snoozeBtn) { snoozeBtn.classList.remove('hidden'); }
        }
        ...
        ['Live', 'Snooze', 'Archive'].forEach(t => { const tabEl = document.getElementById(`tab-${t}`); if (t !== currentTab && tabEl) tabEl.style.display = 'none'; });
        ...
        if (document.getElementById('detail-reply-input')) document.getElementById('detail-reply-input').innerHTML = '';
        ...
        if (document.getElementById('caseDetailView')) document.getElementById('caseDetailView').classList.remove('hidden');
...
window.closeCaseDetail = function() {
    ...
    if (document.getElementById('caseDetailView')) document.getElementById('caseDetailView').classList.add('hidden');
    if (document.getElementById('dashboardView')) document.getElementById('dashboardView').classList.remove('hidden');
    if (document.getElementById('detail-thread-container')) document.getElementById('detail-thread-container').innerHTML = '';
    if (document.getElementById('reply_mention_dropdown')) document.getElementById('reply_mention_dropdown').classList.add('hidden');
    document.querySelectorAll('.inline-mention-dropdown').forEach(d => d.classList.add('hidden'));
    ['Live', 'Snooze', 'Archive'].forEach(t => { const tabEl = document.getElementById(`tab-${t}`); if(tabEl) tabEl.style.display = ''; });
    loadConversations();
};

window.handleReplyFileSelect = function(e) {
    const replInput = document.getElementById('detail-reply-input');
    if (replInput && !replInput.querySelector('.mention-badge')) {
        e.target.value = '';
...
function loadConversations() {
    ...
    if(!feed) return;
    apiCall('getConversations', currentUser).then(data => {
        allCasesData = data || [];
        feed.innerHTML = '';
        ...
        allCasesData.forEach(conv => {
            ...
            if (cardDiv.querySelector('[data-id="conv-id"]')) cardDiv.querySelector('[data-id="conv-id"]').textContent = conv.id;
            if (cardDiv.querySelector('[data-id="subject"]')) cardDiv.querySelector('[data-id="subject"]').textContent = conv.subject;
            if (cardDiv.querySelector('[data-id="details"]')) cardDiv.querySelector('[data-id="details"]').innerHTML = typeof makeLinksClickable === 'function' ? makeLinksClickable(conv.details) : conv.details;
            if (cardDiv.querySelector('[data-id="message"]')) cardDiv.querySelector('[data-id="message"]').innerHTML = typeof makeLinksClickable === 'function' ? makeLinksClickable(conv.message) : conv.message;
            if (cardDiv.querySelector('[data-id="author"]')) cardDiv.querySelector('[data-id="author"]').textContent = creatorName;
            if (cardDiv.querySelector('[data-id="timestamp"]')) cardDiv.querySelector('[data-id="timestamp"]').textContent = new Date(conv.timestamp).toLocaleDateString();
            if (cardDiv.querySelector('[data-id="display-case-id"]')) cardDiv.querySelector('[data-id="display-case-id"]').textContent = conv.id;
            ...
            if (footerActions) {
                ...
                if (currentTab === 'Live' && conv.status !== 'Archived' && !isSnoozed) {
                    if (cbContainer) { cbContainer.classList.remove('hidden'); cbContainer.classList.add('flex'); }
            ...
            if (admCont) conv.admins.forEach(a => { if(a) admCont.innerHTML += window.getMemberBadgeHTML(a, 'Admin', conv.archivedBy, conv.snoozeTime); });
            if (usrCont) conv.users.forEach(u => { if(u) usrCont.innerHTML += window.getMemberBadgeHTML(u, 'User', conv.archivedBy, conv.snoozeTime); });
            ...
            if (wrapperDiv) wrapperDiv.style.display = showInitial ? 'block' : 'none';
            fragment.appendChild(cardFragment);
        });
        feed.appendChild(fragment); window.switchTab(currentTab);
    }).catch(err => { console.error("Error fetching conversations matrix: ", err); });
}

window.setInlineType = function(btn, type) {
    const activeBox = btn.closest('[data-id="inline-reply-box"]');
    if (!activeBox) return;
    const valInput = activeBox.querySelector('.inline-type-val');
    if (valInput) valInput.value = type;
    const replyBtn = activeBox.querySelector('.inline-reply-btn');
    const askBtn = activeBox.querySelector('.inline-ask-btn');
    if (type === 'Reply') {
        if (replyBtn) replyBtn.className = "inline-type-btn inline-reply-btn px-3 py-1 text-[10px] font-bold rounded-md bg-indigo-600 text-white shadow-sm transition-colors";
        if (askBtn) askBtn.className = "inline-type-btn inline-ask-btn px-3 py-1 text-[10px] font-bold rounded-md bg-white/60 text-slate-700 hover shadow-sm transition-colors";
    } else {
        if (askBtn) askBtn.className = "inline-type-btn inline-ask-btn px-3 py-1 text-[10px] font-bold rounded-md bg-red-600 text-white shadow-sm transition-colors";
        if (replyBtn) replyBtn.className = "inline-type-btn inline-reply-btn px-3 py-1 text-[10px] font-bold rounded-md bg-white/60 text-slate-700 hover shadow-sm transition-colors";
    }
};
...
window.toggleInlineReply = function(btn) {
    ...
    if (replyBox.classList.contains('hidden')) {
        ...
        const listEl = replyBox.querySelector('.inline-file-list');
...
window.handleInlineFileSelect = function(e, inputEl) {
    ...
    if (editor && !editor.querySelector('.mention-badge')) {
        e.target.value = '';
        return showCustomDialog("Notice ⚠️", "Pehle kisi ko @mention karein tabhi attachment use kar sakte hain.", false);
    }
    Array.from(e.target.files).forEach(file => {
...
window.submitInlineReply = async function(btn) {
    ...
    const typeVal = replyBox.querySelector('.inline-type-val')?.value || 'Reply';
    btn.disabled = true;
    ...
    try {
        ...
        inputDiv.innerHTML = '';
        inlinePendingFiles = []; 
        const fileListContainer = replyBox.querySelector('.inline-file-list');
...
window.addNewCaseMember = function(name, email, role) {
    ...
    if (document.getElementById('new_case_member_search')) document.getElementById('new_case_member_search').value = '';
    if (document.getElementById('new_case_member_dropdown')) document.getElementById('new_case_member_dropdown').classList.add('hidden');
    window.renderNewCaseMembers();
};

async function loadLabelsForForm() { availableLabels = await apiCall('getLabels'); renderLabels(); populateFilterDropdowns(); }
function renderLabels() { const el = document.getElementById('labels_container'); if (el) el.innerHTML = availableLabels.map(label => `<span onclick="toggleLabel('${label}')" class="cursor-pointer px-3 py-1.5 rounded-full text-xs font-bold transition-all shadow-sm border ${selectedLabels.has(label) ? 'bg-green-800 text-white border-green-900' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}">${label}</span>`).join(''); }
...
window.closeModal = function() { const modal = document.getElementById('appModal'); if (modal) modal.classList.add('hidden'); const form = document.getElementById('convForm'); if (form) form.reset(); };
...
window.handleFormSubmit = async function(e) {
    ...
    try {
        ...
        if (btn) { btn.disabled = false; btn.innerText = 'Post Case'; }
    }
};
...
window.applyFilters = debounce(function() {
    try {
        ...
        if (!feedEl) return;
        Array.from(feedEl.children).forEach(wrapper => {
...
window.openCaseSourceModal = function(event, url) {
    ...
    const iframe = document.getElementById('caseSourceIframe');
    const ntBtn = document.getElementById('caseSourceNewTabBtn');
