...
// ==========================================
const APP_VERSION = "v77"; // 🔥 BUMPED VERSION: Forces devices to refresh and get the new time format
...
// ==========================================
function getWordCount(elementId) {
...
window.validateNewCaseForm = function() {
    ...
    const detailsEl = document.getElementById('f_details_rich');
    const detailsText = detailsEl ? (detailsEl.innerText || detailsEl.textContent || '').trim() : '';
    ...
    if (subject && detailsText && hasLabels && hasMembers) {
...
// Hook validation check to inputs for real-time validation response
document.addEventListener('input', (e) => {
    if (e.target.id === 'f_subject' || e.target.id === 'f_details_rich' || e.target.id === 'f_message_rich') {
...
function renderDashboardPagination(totalItems, totalPages) {
    ...
    if (!container) {
        ...
        container.className = 'flex justify-between items-center bg-white p-3 rounded-xl shadow-sm mb-4 border border-slate-200';
