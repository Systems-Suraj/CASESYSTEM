...
if(document.getElementById('detail-thread-container')) document.getElementById('detail-thread-container').innerHTML =     `<div class="flex justify-center py-10">
...
payloads.forEach(p => {
     ...
     allLoadedComments.push({
         ...
         type: p.mentionType,
         askId: p.mentionType === 'Ask' ? 'NEW' : '', // FIX: Format Ask locally
         status: p.mentionType === 'Ask' ? 'Open' : '', // FIX
...
// FIX: Clear message notifications for this case when replying
...
allLoadedComments.push({
     ...
     attachmentFileName: fileName,
     type: typeVal,
     askId: typeVal === 'Ask' ? 'NEW' : '', // FIX: Format Ask locally
     status: typeVal === 'Ask' ? 'Open' : '', // FIX
...
// Determine ownership for count bifurcation 
let isMyCase = false;
if (currentUser && currentUser.isMaster) {
...
// ==========================================
// NOTIFICATION CLEARING FIXES (Override from Part 1)
// ==========================================
window.openFromNotification = async function(caseId, uniqueId) {
    if (isOpeningCase) return;
    isOpeningCase = true;
